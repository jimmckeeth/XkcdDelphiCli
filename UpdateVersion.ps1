...
# 1. Update src/xkcdversion.pas (can keep ?hash? as it is a Pascal string)
...
# 2. Update all .dproj files in src/ (must use numeric version to avoid compiler error)
...
foreach ($DprojFile in $DprojFiles) {
    ...
    # Use ResourceVersion (all numeric) for the .dproj to keep the compiler happy
    $NewContent = $NewContent -replace "ProductVersion=\d+\.\d+\.\d+\.[^;]+", "ProductVersion=$ResourceVersion"
